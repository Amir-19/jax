...
# ==============================================================================
"""simple one-shot distributed persistent GEMM overlapped with all reduce for H100."""
...
def build_kernel(
  ...
  def _main(ctx, a_device, b_device, c_device, start_sem, done_sem, smem):
    ...
    sem_mc_ptr = ctx.to_remote_multicast_ptr(sem_uc_memref)
    ...
    with ctx.named_region("sync to end kernel"):
      mgpu_utils.warpgroup_barrier()
      with single_thread(scope=ThreadSubset.BLOCK):
        mgpu_utils.spin_lock_multimem_arrive(sem_mc_ptr)
        mgpu_utils.spin_lock_wait(sem_uc_ptr, num_gpus)
...
if __name__ == "__main__":
  ...
  swizzle = (128,)
  stages = (2, 4, 5, 6)
  cta_count = jax.local_devices()[0].core_count
