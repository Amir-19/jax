...
def build_kernel(
  ...
  def _main(ctx, a_device, b_device, c_device, start_sem, done_sem, c_dev_alias, start_sem_alias, done_sem_alias, smem):
    ...
    @worker_for(worker_id=cta, worker_count=mgpu_core.c(cta_count, index), work_count=mgpu_core.c(tile_count, index))
    def body(work_id):
      ...
      bsem_uc_memref = memref_slice(
        start_sem, arith.index_cast(ir.IndexType.get(), cta_idx)
      )
      bsem_mc_ptr = mgpu_utils.to_mc_ptr(
        bsem_uc_memref,
        team_id=0,
      )
      ...
      with ctx.named_region("load reduce and store"):
        mgpu_utils.warpgroup_barrier()
        num_red_elements = mgpu.utils.c(8, i64_ty)
        world_size = mgpu.utils.c(num_gpus, i64_ty)
        work_chunks = arith.divui(mgpu_core.c(tile_n, i64_ty),num_red_elements)
        gpu_work_chunks = arith.divui(work_chunks,world_size)
        device_idx = arith.index_cast(i64_ty,arith.index_cast(index, utils.get_device_id()))
        num_m = mgpu.utils.c(m, index)
        num_tile_m = mgpu_core.c(tile_m, index)
        num_rows_in_tile = arith.minui(num_m, num_tile_m)
        thread_idx = arith.index_cast(index, utils.thread_idx())
        thead_offset_m_start = arith.addi(m_start, thread_idx)
        if_inbound = scf.IfOp(arith.cmpi(arith.CmpIPredicate.ult, thread_idx, num_rows_in_tile), hasElse=False)
        with ir.InsertionPoint(if_inbound.then_block):
          @fori(gpu_work_chunks, None)
          def gpu_loop(i, accs):
            gpu_offset = arith.muli(num_red_elements,arith.addi(device_idx, arith.muli(world_size, i)))
            gpu_offset_n_start = arith.addi(n_start, arith.index_cast(index, gpu_offset))
            uc_memref = memref_slice(c_device, (ds(thead_offset_m_start, 1), ds(gpu_offset_n_start, 1)))
            mc_ptr = mgpu_utils.to_mc_ptr(uc_memref,team_id=0)
    ...
    sem_mc_ptr = mgpu_utils.to_mc_ptr(
      sem_uc_memref,
      team_id=0,
    )
